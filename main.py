...
lene = ''  # используется для набирания символов нужной длины
count = 0  # переменная счетчика
flag = False  # флаг проверки на длина_сообщения(сообщение) mod 64 == 0


def block_slice(count, lene, list_of_bit, blocks, flag):
    for i in list_of_bit:
        tikets = int(len(list_of_bit) / 64)
        id = tikets * 64
    ...
    blocks.append(list_of_bit[id:len(list_of_bit) + 1])
    print(f'Блоки сортированные по 64 бита\n {blocks}\n')
    return [blocks, flag]


blocks, flag = block_slice(count, lene, list_of_bit, blocks, flag)


def fix_last_block(blocks):
    print('Началось выполнение функции по заполнению последнего блока до 64бит')
    last_block = blocks[-1]
    baza_block = 64

    how_much_add = baza_block - len(last_block)
    how_much_add_ = how_much_add
    # while how_much_add != 0:
    #     last_block += '0'
    #     how_much_add -= 1
    blocks[-1] = last_block.rjust(64, '0')
    print(blocks)
    return how_much_add_


def check_flag(flag):
    if flag == True:
        print(f'Обнаружено в последнем блоке меньшее кол-во бит:\n {blocks[-1:]}\n')
        how_much_add = fix_last_block(blocks)
    return how_much_add


how_much_add = check_flag(flag)
...
while count_i_block != 0:
    for i in range(0,count_i_block):
        ...
        t = permute(blocks[i], initial_perm, 64)
        print(f'\nБлок сообщения {i+1}\n{blocks[i]}')
        print(f'Начальная перестановка для блока сообщения \n{t}\n')


        # сдвиг битов в леао на n позиций
        ...
        def encrypt(t, rkb):
            ...
            for i in range(0, 1):
                ...
                # после преобразований с таблицей S, переставляем биты
                ...
                print('После начальной перестановки блок 64 бита участвует в преобразовании Фейстеля')
                print("Идет раунд ", i + 1, " ")
                print('Блок сообщения разбивается на левую и правую часть')
...
# Последняя проверка на добавочные биты
if flag == True:
    prep_fix_count = list_of_CT[-1]
    fix_count = prep_fix_count[0 + how_much_add:64]
    list_of_CT[-1] = fix_count

    cipher_res = ""
    print(f'\nCipher Text blocks:\n{list_of_CT}\n')
    for i in list_of_CT:
        cipher_res += i
    print("Cipher Text in line :", cipher_res)
# если мы не добавляли биты, то сразу выведется шифр текст
else:
    cipher_res = ""
    print(f'\nCipher Text blocks:\n{list_of_CT}\n')
    for i in list_of_CT:
